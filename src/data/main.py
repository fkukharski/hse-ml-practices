...
if np.sum(y.to_numpy()==0) > np.sum(y.to_numpy()==1):
    print(f'Признаков класса 0 в {np.sum(y.to_numpy()==0) /
    	np.sum(y.to_numpy()==1):.2f} раз больше')
else:
    print(f'Признаков класса 1 в {np.sum(y.to_numpy()==1) /
    	np.sum(y.to_numpy()==0):.2f} раз больше')   
...
df_1['num_digits_dom'] = df_1['tfldextract'] \
	.apply(lambda x: len(re.findall('(\d+)', x.domain)))
df_1['num_@'] = df_1['url'].apply(lambda x: x.count('@'))
df_1['num_slash'] = df_1['url'].apply(lambda x: x.count('/'))
df_1['query'] = df_1['url'].apply(urlparse) \
	.apply(lambda x: 0 if x.query == '' else 1)
df_1['caps'] = df_1['tfldextract'] \
	.apply(lambda x: 1 if len(re.compile('[A-Z]+') \
		.findall(x.domain)) > 0 else 0)
...
sns.kdeplot(df_1[df_1['label'] == 0]['url_len'],
	color='g', shade=True, label='Bad')
sns.kdeplot(df_1[df_1['label'] == 1]['url_len'],
	color='b', shade=True, label='good')
...
sns.kdeplot(df_1[df_1['label'] == 0]['num_digits_dom'],
	color='g', shade=True, label='Bad')
sns.kdeplot(df_1[df_1['label'] == 1]['num_digits_dom'],
	color='b', shade=True, label='good')
...
X_train, X_test, y_train, y_test = \
	train_test_split(df['url'], df['label'],
...
X_train_f, X_test_f, y_train_f, y_test_f = \
	train_test_split(df_1.drop(['label'], axis=1), df_1['label'],
		test_size=0.3, random_state=42, stratify=df_1['label'])
...
print(f'Лучшее качество - {roc_auc_score(y_test_f, clf_grid.best_estimator_ \
	.predict_proba(X_test_f)[:,1])}')
...
url_df['path'] = url_df['urlparse'].apply(lambda x: ''. \
	join([s + ' ' for s in x.path.split('/')[1:]]).strip())
url_df['domain'] = url_df['extract'].apply(lambda x: x.domain)
url_df.drop(['extract', 'urlparse'], axis=1, inplace=True)

X_train, X_test, y_train, y_test = train_test_split(url_df, df_1['label'],
	test_size=0.3, random_state=42, stratify=df_1['label'])
...
for i in range(1, 3):
    for k in range(50000, 100001, 10000):
        ...
        log_reg.fit(X_train_tfidf, y_train)
        score.append(recall_score(y_test, log_reg. \
        	predict(X_test_tfidf), pos_label=0))
